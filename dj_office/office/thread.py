from office.models import GlobalVariable, TypeBOND, MoexBOND
            ...
            if cnt_field == 7:
                ...
                try:
                    ...
                    if kup > 0 and kup_year > 0 and nominal > 0:
                        profit = ((kup * kup_year) / nominal) * 100
                    else: continue
                except Exception as e: print(e)
                # Бумаги с низкой доходностью нам не нужны
                if profit < 5: continue
                # Сохраняем бумагу в базу
                obj_type, _ = TypeBOND.objects.get_or_create(typekey=dict_bond['TYPE'])
                bond, _ = MoexBOND.objects.get_or_create(secid=dict_bond['SECID'], typekey=obj_type)
                bond.name = dict_bond['NAME']
                bond.matdate = datetime.strptime(dict_bond['MATDATE'], '%Y-%m-%d')
                bond.facevalue = dict_bond['FACEVALUE']
                bond.couponfrequency = dict_bond['COUPONFREQUENCY']
                bond.couponvalue = dict_bond['COUPONVALUE']
                bond.profit = profit
                bond.save()
