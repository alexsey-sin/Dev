...
from api.models import TxvRostelecom
...
def set_txv_rostelecom(request):
...
def get_txv_rostelecom(request):
    if request.method == 'GET':
        key = request.GET.get('key')
        if key != 'Q8kGM1HfWz':
            HttpResponse('ERROR key.', status=status.HTTP_403_FORBIDDEN)
        
        tmp_txvs = TxvRostelecom.objects.filter(status=0)
...
@csrf_exempt
def set_txv_rostelecom_status(request):
    if request.method == 'POST':
        ...
        try:
            ins_txv = TxvRostelecom.objects.get(id=id_txv)
