...
def get_txv_mts(request):
    if request.method == 'GET':
        key = request.GET.get('key')
        if key != 'Q8kGM1HfWz':
            HttpResponse('ERROR key.', status=status.HTTP_403_FORBIDDEN)
        
        tmp_txvs = TxvMts.objects.filter(status=0)
...
@csrf_exempt
def set_txv_mts_status(request):
    if request.method == 'POST':
        ...
        new_mts_address = data.get('mts_address')
        new_status = data.get('status')
        new_bot_log = data.get('bot_log')
        try:
            ins_txv = TxvMts.objects.get(id=id_txv)
            ...
            if new_mts_address: ins_txv.mts_address = new_mts_address
