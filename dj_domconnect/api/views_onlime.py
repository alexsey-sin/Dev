...
from api.models import BidOnlime, BotAccess
...
def set_bid_onlime(request):
    if request.method == 'GET':
        ...
        try:
            ...
            # Возьмем доступы
            obj_visit, _ = BotAccess.objects.get_or_create(name='Бот автозаявки ОнЛайм')
            ...
            entrances = request.GET.get('entrances')
            if entrances == None or len(entrances) == 0: raise ValueError('entrances is absent')
            bid.entrances = entrances

            floor = request.GET.get('floor')
            if floor == None or len(floor) == 0: raise ValueError('floor is absent')
            bid.floor = floor

            tarifs_eq = request.GET.get('tarifs_eq')
            if tarifs_eq: bid.tarifs_eq = tarifs_eq
...
def get_bid_onlime(request):
    if request.method == 'GET':
        ...
        if yes_work == True:
            tmp_bids = BidOnlime.objects.filter(status=0).order_by('-pub_date')[:5]
...
def set_bid_onlime_status(request):
    if request.method == 'GET':
        key = request.GET.get('key')
        if key != 'Q8kGM1HfWz':
        ...
        try:
            ins_bid = BidOnlime.objects.get(id=id_bid)
