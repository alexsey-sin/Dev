...
from api.models import BidDomRu, BotAccess
...
def set_bid_domru(request):
    if request.method == 'GET':
        ...
        try:
            ...
            # Возьмем доступы
            obj_visit, _ = BotAccess.objects.get_or_create(name='Бот автозаявки ДомРу')
...
def get_bid_domru(request):
    if request.method == 'GET':
        ...
        if yes_work == True:
            tmp_bids = BidDomRu.objects.filter(status=0).order_by('-pub_date')[:5]
